...
What Freund found had been released on 24 February as version 5.6.0 of xz-utils, and again on 9 March as 5.6.1. The project's own account says both release files were created and signed by Jia Tan. A timeline of the case by Russ Cox dates Jia Tan's first message to the project's mailing list to October 2021. The project also says the code that switched the backdoor on was never in its Git repository, the public history of its source code.
...
The slow login was not the first sign. Cox's timeline records that valgrind errors began turning up in Red Hat's distributions, Fedora among them, on 4 March. They did not go unanswered. On 8 March Jia Tan committed what Cox calls a "purported Valgrind fix", "a misdirection, but an effective one". On 9 March the reworked backdoor went out as 5.6.1, which Cox calls "the actual Valgrind fix". Freund's post confirms it from the other side: the injected code caused valgrind errors in some configurations, 5.6.1 tried to work around them, and the person behind it wrote to various lists about the "fixes".
